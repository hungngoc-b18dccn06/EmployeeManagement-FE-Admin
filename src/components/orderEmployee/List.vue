...
<template>
    <div class="grid">
        <div class="col-12">
            <div class="card">
                <h2>LG Ecommerce</h2>
                <DataView :value="storeProduct.getProducts" :layout="layout" :paginator="true" :rows="9" :sortOrder="sortOrder" :sortField="sortField">
                    <template #header>
                    ...
                    <template #list="slotProps">
                        <div class="col-12">
                            <div class="flex flex-column md:flex-row align-items-center p-3 w-full" v-for="(item, index) in slotProps.items" :key="index">
                                <img :src="item.productImage" :alt="item.productName" class="my-4 md:my-0 w-9 md:w-10rem shadow-2 mr-5" />
                            ...
                            <div class="col-12 md:col-4" v-for="(item, index) in slotProps.items" :key="index" >
                                <div class="card m-3 border-1 surface-border" >
                                    ...
                                    <div class="text-center">
                                        <img :src="item.productImage" :alt="item.productName" class="w-9 shadow-2 my-3 mx-0" />
                                        <div class="text-2xl font-bold">{{ item.productName }}</div>
                                        <div class="mb-3">{{ item.description }}</div>
                                        <Rating :modelValue="item.average_rating" :readonly="true" :cancel="false"></Rating>
                                    </div>
                                    <div class="flex align-items-center justify-content-between">
                                        <span class="text-2xl font-semibold">${{ item.price }}</span>
                                        <Button icon="pi pi-shopping-cart" :disabled="item.status === 'OUTOFSTOCK'"></Button>
                                    </div>
                                </div>
                            </div>
                        </div>
